...
const Avarege=()=>{
    const [fsma, setfsma] = React.useState("sma_10");
    const [cross, setcross] = React.useState("DESC");
    const [lsma, setlsma] = React.useState("sma_50");
    const [avaregedata,setavaregedata]=React.useState([])

    const onMovingfilter=async()=>{

      if(cross==="DESC"){
      const avrege=await axios.get(`https://trendlyne.com/fundamentals/all-in-one-screener-data-get/?perPageCount=25&pageNumber=0&query=${fsma}+%3E+${lsma}&columns=${fsma}%2C${lsma}%2CcurrentPrice%2CMCAP_Q%2Cday_changeP%2Cyear_changeP%2Cyear_high%2Cyear_low%2Csma_200&groupType=all&groupName=&sortBy=${fsma}&order=${cross}`)
      console.log('log===>',avrege);
      setavaregedata(avrege.data.body.tableData)
      }else{
        const avareges=await axios.get(`https://trendlyne.com/fundamentals/all-in-one-screener-data-get/?perPageCount=25&pageNumber=0&query=${fsma}+%3C+${lsma}&columns=${fsma}%2C${lsma}%2CcurrentPrice%2CMCAP_Q%2Cday_changeP%2Cyear_changeP%2Cyear_high%2Cyear_low%2Csma_200&groupType=all&groupName=&sortBy=${fsma}&order=${cross}`)
    ...
    <div className="container mt-5 avregecontent mb-5">
      <div>
        <CustomSelect value={fsma} onChange={setfsma}>
          <StyledOption value={"sma_5"}>SMA5</StyledOption>
          <StyledOption value={"sma_10"}>SMA10</StyledOption>
          <StyledOption value={"sma_20"}>SMA20</StyledOption>
          <StyledOption value={"sma_30"}>SMA30</StyledOption>
          <StyledOption value={"sma_50"}>SMA50</StyledOption>
          <StyledOption value={"sma_100"}>SMA100</StyledOption>
          <StyledOption value={"sma_150"}>SMA150</StyledOption>
          <StyledOption value={"sma_200"}>SMA200</StyledOption>
        </CustomSelect>
      </div>
      <div>
